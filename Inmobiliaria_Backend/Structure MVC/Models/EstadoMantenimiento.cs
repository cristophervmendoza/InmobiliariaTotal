...
namespace backend_csharpcd_inmo.Structure_MVC.Models
{
    ...
    public class EstadoMantenimiento : IValidatableObject
    {
        [Key]
        ...
        public virtual System.Collections.Generic.ICollection<Mantenimiento> Mantenimientos { get; set; }
        ...
        public System.Collections.Generic.IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (!string.IsNullOrEmpty(Nombre))
            {
                ...
                {
                    yield return new ValidationResult("El estado del mantenimiento no es válido. Estados permitidos: Pendiente, Programado, En Proceso, Completado, Cancelado, Suspendido, Reprogramado, En Revision, Aprobado, Rechazado", new[] { nameof(Nombre) });
                }

                var palabrasProhibidas = new[] { "test", "prueba", "ejemplo", "xxx", "temporal", "temp", "demo" };
            ...
            {
                yield return new ValidationResult("Los estados de mantenimiento son datos de catálogo que deberían existir desde el inicio del sistema", new[] { nameof(CreadoAt) });
            }
